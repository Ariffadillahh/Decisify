  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-5 border-b border-gray-50 flex items-center justify-between">
        ...
        <Link to="/notes" className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 transition-colors">
      ...
      <div className="divide-y divide-gray-50">
        {recentNotes.length > 0 ? (
          recentNotes.map((note) => (
            <Link key={note.id} to={`/notes/${note.id}`} onClick={() => setActiveNote(note)} className="p-4 flex items-center gap-4 hover:bg-gray-50 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-bold text-gray-700 truncate group-hover:text-indigo-600 transition-colors">{note.title || "Untitled Note"}</h4>
                <p className="text-[11px] text-gray-400 mt-0.5">Diperbarui {new Date(note.updatedAt).toLocaleDateString("id-ID", { day: "numeric", month: "short" })}</p>
              </div>
            </Link>
          ))
        ) : (
